...
def cal_pesq(dirty_wav, clean_wav, FS=16000):
    try:
...
def cal_pesq_batch(dirty_wavs, clean_wavs, FS=16000):
...
def cal_stoi_batch(dirty_wavs, clean_wavs, FS=16000):
...
def cal_mos_batch(dirty_wavs, clean_wavs):
    # fp = open('./mos_socres.txt', 'w')
    # fp2 = open('./pesq_socres.txt', 'w')
    ...
    for i in range(len(dirty_wavs)):
        csig_score, cbak_score, covl_score, pesq_score, ssnr_score = CompositeEval(clean_wavs[i], dirty_wavs[i])
        ...
        # fp.write('[{}] CSIG {:.4f} CBAK {:.4f} COVL {:.4f} SSNR {:.4f}\n'.format(i+1, csig_score, cbak_score, covl_score, ssnr_score))
        # fp2.write('[{}] PESQ {:.4f}\n'.format(i + 1, pesq_score))
    avg_csig_score /= len(dirty_wavs)
    avg_cbak_score /= len(dirty_wavs)
    avg_covl_score /= len(dirty_wavs)
    # avg_pesq_score /= len(dirty_wavs)
    avg_ssnr_score /= len(dirty_wavs)
    # return avg_csig_score, avg_cbak_score, avg_covl_score, avg_pesq_score, avg_ssnr_score
    # fp.close()
    # fp2.close()
    return avg_csig_score, avg_cbak_score, avg_covl_score, avg_ssnr_score
